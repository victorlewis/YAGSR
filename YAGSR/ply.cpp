...
// define DBG() to print debug messages to the console.
#ifdef _DEBUG
...
bool Ply::ParseHeader(std::ifstream& plyFile)
{
    ...
    {
        DBG("Unexpected error reading next line\n");
        return false;
    }
    if (token1 != "ply")
    {
        DBG("Invalid ply file\n");
        return false;
    }
    ...
    {
        DBG("Invalid ply file, expected format\n");
        return false;
    }
    if (token1 != "format binary_little_endian 1.0")
    {
        DBG("Unsupported ply file, only binary_little_endian supported\n");
        return false;
    }
    ...
    {
        DBG("Invalid ply file, expected \"element vertex {number}\"\n");
        return false;
    }
    ...
    while (true)
    {
        if (!GetNextPlyLine(plyFile, line))
        {
            DBG("unexpected error reading line\n");
            return false;
        }
        ...
        if (token1 != "property")
        {
            DBG("Invalid header, expected property\n");
            return false;
        }
        ...
        {
            DBG("Unsupported type \"%s\" for property \"%s\"\n", token2.c_str(), token3.c_str());
            return false;
        }
    }
